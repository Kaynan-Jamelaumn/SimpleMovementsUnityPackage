...
#pragma warning restore CS1998
    ...
    protected virtual IEnumerator SetBulletLikeTargetLaunchRoutine(AbilityHolder ability, Transform playerTransform, GameObject instantiatedParticle, AbilityStateMachine abilityStateMachine, AttackCast attackCast = null)
    {
        //ability.abilityState = AbilityHolder.AbilityState.Launching;
        abilityStateMachine.TransitionToState(AbilityStateMachine.EAbilityState.Launching);
        abilitiesStateMachine.TransitionToState(AbilitiesStateMachine.EAbilitiesState.Available);
        float startTime = Time.time;
    ...
    protected virtual async Task SetAbilityActions(AbilityHolder ability, Transform abilityTargetTransform, AbilityStateMachine abilityStateMachine, AttackCast attackCast = null)
    {
        Transform targetedTransform = abilityTargetTransform;
        GameObject instantiatedParticle = Instantiate(ability.particle);
        if (!ability.abilityEffect.isFixedPosition && ability.abilityEffect.shouldMarkAtCast)
        ...
        if (ability.abilityEffect.castDuration != 0)
        {
            ...
            if (ability.abilityEffect.isPartialPermanentTargetWhileCasting)
                StartCoroutine(SetPermanentTargetOnCastRoutine(ability, targetedTransform, instantiatedParticle, abilityStateMachine, attackCast));
            else
                StartCoroutine(SetTargetOnCastRoutine(ability, targetedTransform, instantiatedParticle, abilityStateMachine, attackCast));
        }
        else
        {
            if (ability.abilityEffect.isPermanentTarget)
                StartCoroutine(SetPermanentTargetLaunchRoutine(ability, targetedTransform, instantiatedParticle, abilityStateMachine, attackCast));
            else
                StartCoroutine(DelayedSetTargetLaunchRoutine(ability, targetedTransform, instantiatedParticle, abilityStateMachine, attackCast));
        }
    }



    public virtual IEnumerator SetPermanentTargetLaunchRoutine(AbilityHolder ability, Transform playerTransform, GameObject instantiatedParticle, AbilityStateMachine abilityStateMachine, AttackCast attackCast = null)
    {
        //ability.abilityState = AbilityHolder.AbilityState.Launching;
        abilityStateMachine.TransitionToState(AbilityStateMachine.EAbilityState.Launching);
        abilitiesStateMachine.TransitionToState(AbilitiesStateMachine.EAbilitiesState.Available);
        ...
        ApplyAbilityUse(ability, abilityStateMachine, attackCast, instantiatedParticle);
    }

    public virtual IEnumerator DelayedSetTargetLaunchRoutine(AbilityHolder ability, Transform playerTransform, GameObject instantiatedParticle, AbilityStateMachine abilityStateMachine, AttackCast attackCast = null)
    {
        if (!ability.abilityEffect.shouldMarkAtCast) SetGizmosAndColliderAndParticlePosition(ability, playerTransform, instantiatedParticle);
        //ability.abilityState = AbilityHolder.AbilityState.Launching;
        abilityStateMachine.TransitionToState(AbilityStateMachine.EAbilityState.Launching);
    ...
    public virtual void ApplyAbilityUse(AbilityHolder ability, AbilityStateMachine abilityStateMachine, AttackCast attackCast = null, GameObject instantiatedParticle = null, GameObject affectedTarget = null)
    {
        //ability.abilityEffect.Use();
        foreach (var effect in ability.abilityEffect.effects)
        {
        ...
        //ability.abilityState = AbilityHolder.AbilityState.Active;
        abilityStateMachine.TransitionToState(AbilityStateMachine.EAbilityState.Active);
        ability.activeTime = Time.time;
        targetTransform = transform;
        StartCoroutine(ActiveAbilityRoutine(ability, abilityStateMachine, instantiatedParticle));
    }

    public virtual IEnumerator CooldownAbilityRoutine(AbilityHolder ability, AbilityStateMachine abilityStateMachine)
    {
        yield return new WaitForSeconds(ability.abilityEffect.coolDown);
        abilityStateMachine.TransitionToState(AbilityStateMachine.EAbilityState.Ready);
        //ability.abilityState = AbilityHolder.AbilityState.Ready;
    }

    public virtual IEnumerator ActiveAbilityRoutine(AbilityHolder ability, AbilityStateMachine abilityStateMachine, GameObject instantiatedParticle = null)
    {
        yield return new WaitForSeconds(ability.abilityEffect.duration);
        //ability.abilityState = AbilityHolder.AbilityState.InCooldown;
        abilityStateMachine.TransitionToState(AbilityStateMachine.EAbilityState.InCooldown);
        StartCoroutine(CooldownAbilityRoutine(ability, abilityStateMachine));
        if (instantiatedParticle) Destroy(instantiatedParticle);
    }

    public virtual IEnumerator SetPermanentTargetOnCastRoutine(AbilityHolder ability, Transform playerTransform, GameObject instantiatedParticle, AbilityStateMachine abilityStateMachine, AttackCast attackCast = null)
    {
        //ability.abilityState = AbilityHolder.AbilityState.Casting;
        //abilityStateMachine.TransitionToState(AbilityStateMachine.EAbilityState.Casting);
        float startTime = Time.time;
        ...
        SetGizmosAndColliderAndParticlePosition(ability, playerTransform, instantiatedParticle, false);
        if (ability.abilityEffect.isPermanentTarget)
            StartCoroutine(SetPermanentTargetLaunchRoutine(ability, playerTransform, instantiatedParticle, abilityStateMachine, attackCast));
        else
            StartCoroutine(DelayedSetTargetLaunchRoutine(ability, playerTransform, instantiatedParticle, abilityStateMachine, attackCast));
    }

    public virtual IEnumerator SetTargetOnCastRoutine(AbilityHolder ability, Transform playerTransform, GameObject instantiatedParticle, AbilityStateMachine abilityStateMachine, AttackCast attackCast = null)
    {
        //ability.abilityState = AbilityHolder.AbilityState.Casting;
        //abilityStateMachine.TransitionToState(AbilityStateMachine.EAbilityState.Casting);
        if (ability.abilityEffect.shouldMarkAtCast == true) SetGizmosAndColliderAndParticlePosition(ability, playerTransform, instantiatedParticle);
        ...
        while (Time.time <= startTime + ability.abilityEffect.castDuration)
            yield return null;
        if (ability.abilityEffect.shouldLaunch)
            StartCoroutine(SetBulletLikeTargetLaunchRoutine(ability, playerTransform, instantiatedParticle, abilityStateMachine, attackCast));
        else if (!ability.abilityEffect.shouldLaunch && ability.abilityEffect.isPermanentTarget)
            StartCoroutine(SetPermanentTargetLaunchRoutine(ability, playerTransform, instantiatedParticle, abilityStateMachine, attackCast));
        else
            StartCoroutine(DelayedSetTargetLaunchRoutine(ability, playerTransform, instantiatedParticle, abilityStateMachine, attackCast));
    ...
    public virtual IEnumerator UntilReachesPosition(AbilityHolder ability, Transform playerTransform, GameObject instantiatedParticle, AbilityStateMachine abilityStateMachine, AttackCast attackCast = null)
    {
        //ability.abilityState = AbilityHolder.AbilityState.Launching;
        abilityStateMachine.TransitionToState(AbilityStateMachine.EAbilityState.Launching);
        abilitiesStateMachine.TransitionToState(AbilitiesStateMachine.EAbilitiesState.Available);

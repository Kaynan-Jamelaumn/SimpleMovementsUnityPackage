...
public static class RiverGenerator
{
        ...
        WaterBodyType mouth, LakeFeature receivingLake, LakeFeature terminal, float phase, WaterSettings s, TerrainHeightSampler sampler)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            river.HalfWidth[i] = Mathf.Max(0.75f, width * 0.5f);
            depth[i] = s.RiverDepth * Mathf.Lerp(0.4f, 1f, progress);
        }
        ...
        for (int i = 0; i < n; i++)
        {
            if (!insideSource[i])
            {
                ...
                float offset = river.HalfWidth[i] + 2f;
        ...
        for (int i = 0; i < n; i++)
            river.Surface[i] = Mathf.Max(river.Surface[i], receivingLevel);
